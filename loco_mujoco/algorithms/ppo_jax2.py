...
class PPOAgentConf(AgentConfBase):
    ...
    tx: Any
    # KL 自适应学习率相关参数
    kl_adaptive_lr_active: bool = False
    kl_desired: float = 0.01
    lr_factor: float = 1.2
    lr_min: float = 1e-5
    lr_max: float = 1e-2
    ...
    def serialize(self):
        ...
        return {
            ...
            "kl_adaptive_lr_active": self.kl_adaptive_lr_active,
            "kl_desired": self.kl_desired,
    ...
    def from_dict(cls, d):
        ...
        tx = cls._get_optimizer(config)  # 修复：调用 cls 而不是 PPOJax
        return cls(
            config=config,
            network=flax.serialization.from_state_dict(ActorCritic, d["network"]),
            tx=tx,
            kl_adaptive_lr_active=d.get("kl_adaptive_lr_active", False),
            kl_desired=d.get("kl_desired", 0.01),
            lr_factor=d.get("lr_factor", 1.2),
            lr_min=d.get("lr_min", 1e-5),
            lr_max=d.get("lr_max", 1e-2)
        )
...
class PPOAgentState(AgentStateBase):
    ...
    def from_dict(cls, d, agent_conf):
        ...
        return cls(
            ...
            current_lr=d.get("current_lr", agent_conf.config.lr)
        )
...
class PPOJax(JaxRLAlgorithmBase):
    ...
    def init_agent_conf(cls, env, config):
        ...
        # 初始化 KL 自适应学习率参数
        kl_adaptive_lr_active = False
        kl_desired = 0.01
        lr_factor = 1.2
        lr_min = 1e-5
        lr_max = 1e-2
        
        if hasattr(config.experiment, 'kl_adaptive_lr') and config.experiment.kl_adaptive_lr.active:
            kl_adaptive_lr_active = True
            kl_desired = config.experiment.kl_adaptive_lr.desired_kl
            lr_factor = config.experiment.kl_adaptive_lr.factor
            lr_min = config.experiment.kl_adaptive_lr.min_lr
            lr_max = config.experiment.kl_adaptive_lr.max_lr
        ...
        #     # 新增：网络架构增强参数
        #     use_residual=getattr(config.experiment, 'use_residual', False),
        #     use_attention=getattr(config.experiment, 'use_attention', False),
        #     attention_embed_dim=getattr(config.experiment, 'attention_embed_dim', 64)
        # )

# 12-8注意力机制改进
        # 从配置中获取身体部位分组信息
        body_part_indices = None
        if getattr(config.experiment, 'use_grouped_attention', False):
            # 定义身体部位分组（根据29自由度的G1机器人）
            # 这里需要根据实际观测维度调整索引
            # 假设观测包含关节位置和速度，需要分别处理
            body_part_indices = getattr(config.experiment, 'body_part_indices', None)
            if body_part_indices is None:
                # 默认分组（示例，需要根据实际观测调整）
                body_part_indices = {
                    'legs': list(range(0, 12)),      # 腿部关节
                    'arms': list(range(12, 24)),     # 手臂关节
                    'torso': list(range(24, 29))     # 躯干关节
                }
        
        # 使用改进的网络
        use_improved_network = getattr(config.experiment, 'use_improved_network', False)
        
        if use_improved_network:
            network = ImprovedActorCritic(
                env.info.action_space.shape[0],
                activation=config.experiment.activation,
                init_std=config.experiment.init_std,
                learnable_std=config.experiment.learnable_std,
                ...
                use_attention=getattr(config.experiment, 'use_attention', False),
                attention_embed_dim=getattr(config.experiment, 'attention_embed_dim', 64),
                use_channel_attention=getattr(config.experiment, 'use_channel_attention', False),
                channel_attention_layers=tuple(getattr(config.experiment, 'channel_attention_layers', [0, 2])),
                channel_reduction_ratio=getattr(config.experiment, 'channel_reduction_ratio', 16),
                use_grouped_attention=getattr(config.experiment, 'use_grouped_attention', False),
        ...
        return cls._agent_conf(
            config, network, tx,
            kl_adaptive_lr_active, kl_desired, lr_factor, lr_min, lr_max
        )
    ...
    @classmethod
    def _compute_kl_divergence(cls, old_log_prob, new_log_prob):
        """
        计算新旧策略之间的 KL 散度
        
        Args:
            old_log_prob: 旧策略的对数概率
            new_log_prob: 新策略的对数概率
            
        Returns:
            KL 散度
        """
        # KL 散度近似计算：mean(log(old_prob) - log(new_prob))
        kl = jnp.mean(old_log_prob - new_log_prob)
        return kl
    ...
    def _train_fn(cls, rng, env,
        ...
        if agent_state is not None:
            ...
            current_lr = agent_state.current_lr
        else:
            train_state = None
        ...
        def _update_step(runner_state, unused):
            ...
            def _env_step(runner_state, unused):
                train_state, env_state, last_obs, train_state_buffer, rng, current_lr = runner_state
                ...
                runner_state = (train_state, env_state, obsv, train_state_buffer, rng, current_lr)
            ...
            def _update_epoch(update_state, unused):
                def _update_minbatch(train_state_and_lr, batch_info):
                    train_state, current_lr = train_state_and_lr
                    ...
                    def _loss_fn(params, traj_batch, gae, targets):
                        ...
                        return total_loss, (value_loss, loss_actor, entropy, log_prob)

                    grad_fn = jax.value_and_grad(_loss_fn, has_aux=True)
                    total_loss, (grads, (value_loss, loss_actor, entropy, new_log_prob)) = grad_fn(
                        train_state.params, traj_batch, advantages, targets
                    )
                    
                    # # KL 自适应学习率调整
                    # if agent_conf.kl_adaptive_lr_active:
                    #     kl_div = cls._compute_kl_divergence(traj_batch.log_prob, new_log_prob)
                    #     kl_mean = jnp.mean(kl_div)
                        
                    #     # 根据 KL 调整学习率
                    #     new_lr = current_lr
                    #     factor = agent_conf.lr_factor
                        
                    #     def _update_lr_high_kl():
                    #         return jnp.maximum(agent_conf.lr_min, current_lr / factor)
                        
                    #     def _update_lr_low_kl():
                    #         return jnp.minimum(agent_conf.lr_max, current_lr * factor)
                        
                    #     def _keep_lr():
                    #         return current_lr
                        
                    #     new_lr = jax.lax.cond(
                    #         kl_mean > agent_conf.kl_desired * 2.0,
                    #         _update_lr_high_kl,
                    #         lambda: jax.lax.cond(
                    #             kl_mean < agent_conf.kl_desired / 2.0,
                    #             _update_lr_low_kl,
                    ...
                    # return (train_state, current_lr), total_loss
                    ...
                    # ⭐ 新增：根据 current_lr 相对 base_lr 的比例来缩放梯度
                    scale = current_lr / base_lr
                    scaled_grads = jax.tree.map(lambda g: g * scale, grads)

                    # ⭐ 修改：用缩放后的梯度更新参数
                    train_state = train_state.apply_gradients(grads=scaled_grads)

                    return (train_state, current_lr), total_loss


                train_state_and_lr, traj_batch, advantages, targets, rng = update_state
                rng, _rng = jax.random.split(rng)
                ...
                train_state_and_lr, total_loss = jax.lax.scan(
                    _update_minbatch, train_state_and_lr, minibatches
                )
                update_state = (train_state_and_lr[0], traj_batch, advantages, targets, rng, train_state_and_lr[1])
                return update_state, total_loss

            update_state = (train_state, traj_batch, advantages, targets, rng, current_lr)
            update_state, loss_info = jax.lax.scan(
                _update_epoch, update_state, None, config.update_epochs
            )
            train_state = update_state[0]
            current_lr = update_state[-1]
            rng = update_state[-2]
            ...
            def _evaluation_step():
                ...
                obsv, env_state = env.reset(reset_rng)
                runner_state_eval = (train_state, env_state, obsv, train_state_buffer, rng, current_lr)
            ...
            runner_state = (train_state, env_state, last_obs, train_state_buffer, rng, current_lr)
            return runner_state, (metric, validation_metrics)

        rng, _rng = jax.random.split(rng)
        runner_state = (train_state, env_state, obsv, train_state_buffer, _rng, current_lr)
        runner_state, metrics = jax.lax.scan(
            _update_step, runner_state, None, config.num_updates
        )

        agent_state = cls._agent_state(train_state=runner_state[0], current_lr=runner_state[-1])
