...
class GoalTrajMimic(Goal):
    ...
    def __init__(self, info_props: Dict,
        ...
        # ⭐ 处理sites_for_mimic参数
        if "sites_for_mimic" in kwargs:
            info_props = dict(info_props)  # 创建副本避免修改原始数据
            info_props["sites_for_mimic"] = kwargs["sites_for_mimic"]
            del kwargs["sites_for_mimic"]  # 从kwargs中移除，避免传递给父类
    ...
    def _init_from_mj(self,
        ...
        # ⭐ 添加调试信息 ⭐
        print(f"GoalTrajMimic dim calculation:")
        print(f"  n_joints: {n_joints}")
        print(f"  n_sites: {n_sites}")
        print(f"  n_step_lookahead: {self.n_step_lookahead}")
        print(f"  size_for_joint_pos: {size_for_joint_pos}")
        print(f"  size_for_joint_vel: {size_for_joint_vel}")
        print(f"  size_for_sites: {size_for_sites}")
