...
class MimicReward(TrajectoryBasedReward):
    ...
    def __init__(self, env: Any,
        ...
        # 新增：上半身和腰部稳定性权重
        self._upper_body_stability_w = kwargs.get("upper_body_stability_w", 0.0)
        self._waist_stability_w = kwargs.get("waist_stability_w", 0.0)
    ...
    def __call__(self,
        ...
        # 新增：计算上半身稳定性奖励
        upper_body_stability_reward = 0.0
        if self._upper_body_stability_w > 0.0 and len(self._rel_site_ids) > 1:
            # 找到上半身site（通常是名为upper_body_mimic的site）
            upper_body_idx = -1  # 初始化为-1
            for i, site_name in enumerate(self._info_props["sites_for_mimic"]):
                if "upper_body" in site_name:
                    # 由于main_site被移除，索引需要调整
                    # 如果upper_body_mimic是第一个site，那么在删除main_site后，它在site_rangles中的索引是0
                    upper_body_idx = i - 1 if i > 0 else 0
                    break
            
            if upper_body_idx >= 0 and upper_body_idx < len(site_rangles):
                # 惩罚上半身的角度变化（相对于轨迹的角度稳定性）
                upper_body_angle_dist = backend.sum(backend.square(site_rangles[upper_body_idx] - site_rangles_traj[upper_body_idx]))
                # 也可以惩罚上半身的角速度
                upper_body_angvel_dist = backend.sum(backend.square(site_rvel[upper_body_idx, :3] - site_rvel_traj[upper_body_idx, :3]))
                
                # 组合角度稳定性和角速度稳定性
                stability_dist = upper_body_angle_dist + 0.1 * upper_body_angvel_dist
                upper_body_stability_reward = backend.exp(-10.0 * stability_dist)  # 使用指数衰减奖励

        # 新增：计算腰部稳定性奖励
        waist_stability_reward = 0.0
        if self._waist_stability_w > 0.0:
            # 识别腰部关节（pelvis相关的关节）
            waist_joint_names = ["pelvis_tilt", "pelvis_list", "pelvis_rotation"]
            waist_qvel_indices = []
            
            # 找到腰部关节在qvel中的索引
            for i in range(model.njnt):
                joint_name = mujoco.mj_id2name(model, mujoco.mjtObj.mjOBJ_JOINT, i)
                if joint_name in waist_joint_names:
                    qvel_indices = mj_jntid2qvelid(i, model)
                    waist_qvel_indices.extend(qvel_indices)
            
            if waist_qvel_indices:
                waist_qvel_indices = np.array(waist_qvel_indices)
                # 计算腰部关节速度的平方和（惩罚过大的腰部运动）
                waist_vel_norm = backend.sum(backend.square(data.qvel[waist_qvel_indices]))
                # 相对于轨迹的腰部速度偏差
                waist_vel_traj_norm = backend.sum(backend.square(traj_data.qvel[waist_qvel_indices]))
                waist_vel_diff = backend.abs(waist_vel_norm - waist_vel_traj_norm)
                
                # 使用指数衰减奖励：速度越接近轨迹，奖励越高
                waist_stability_reward = backend.exp(-5.0 * waist_vel_diff)
        ...
        if len(self._rel_site_ids) > 1:
            total_reward = (total_reward
                        ...
                        + self._upper_body_stability_w * upper_body_stability_reward  # 新增上半身稳定
                        + self._waist_stability_w * waist_stability_reward)  # 新增腰部稳定性奖励
