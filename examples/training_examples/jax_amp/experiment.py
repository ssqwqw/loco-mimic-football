...
def experiment(config: DictConfig):
    try:
        print("=" * 70)
        print("LOADING MULTIPLE TRAJECTORIES")
        print("=" * 70)
        ...
        for traj_file in traj_files:
            if os.path.exists(traj_file):
                ...
                print(f"✓ Loaded trajectory from: {os.path.basename(traj_file)}")
...
# 修改第 73-75 行为：
        ...
        # ⭐ 创建环境（使用合并后的轨迹）
        # 安全地获取 task_factory.params，如果不存在则使用空字典
        task_factory_params = {}
        if hasattr(config.experiment.task_factory, 'params') and config.experiment.task_factory.params is not None:
            task_factory_params = dict(config.experiment.task_factory.params)
            # 如果存在 default_dataset_conf，移除它（因为我们使用自定义数据集）
            if 'default_dataset_conf' in task_factory_params:
                task_factory_params.pop('default_dataset_conf')
        
        env = factory.make(**config.experiment.env_params, **task_factory_params,
                          custom_dataset_conf=CustomDatasetConf(combined_traj))

        # create expert dataset from the loaded trajectory
        ...
        if not config.experiment.debug:
            ...
            for i in range(len(training_metrics.mean_episode_return)):
                ...
                if (i+1) % config.experiment.validation_interval == 0 and config.experiment.validation.active:
                    ...
                    site_rrotvec = validation_metrics.euclidean_distance.site_rrotvec[i] if hasattr(validation_metrics.euclidean_distance, 'site_rrotvec') else validation_metrics.euclidean_distance.site_rpos[i]
                    site_rvel = validation_metrics.euclidean_distance.site_rvel[i] if hasattr(validation_metrics.euclidean_distance, 'site_rvel') else validation_metrics.euclidean_distance.site_rpos[i]
